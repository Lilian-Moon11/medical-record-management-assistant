...
def get_health_record_view(page: ft.Page):
    ...
    sections_column = ft.Column(spacing=pt_scale(page, 12))
    ...
    def rebuild_sections():
        ...
        if all_list or not st:
            allergies_panel = themed_panel(
                ...
                ListEditorBody(
                    ...
                    on_save=lambda items: upsert_patient_field_value(page.db_connection, patient_id, allergies_key, json.dumps(items), _list_meta(allergies_key)[0]),
            ...
            meds_panel = themed_panel(
                ...
                ListEditorBody(
                    ...
                    on_save=lambda items: upsert_patient_field_value(
                        page.db_connection, patient_id, meds_key, json.dumps(items), _list_meta(meds_key)[0]
                    ),
        ...
        if cond_list or not st:
            conditions_panel = themed_panel(
                ...
                ListEditorBody(
                    ...
                    on_save=lambda items: upsert_patient_field_value(
                        page.db_connection, patient_id, conditions_key, json.dumps(items), _list_meta(conditions_key)[0]
                    ),
        ...
        if surg_list or not st:
            surgeries_panel = themed_panel(
                ...
                ListEditorBody(
                    ...
                    on_save=lambda items: upsert_patient_field_value(
                        page.db_connection, patient_id, surgeries_key, json.dumps(items), _list_meta(surgeries_key)[0]
                    ),
        ...
        if ins_list or not st:
            insurance_panel = themed_panel(
                ...
                ListEditorBody(
                    ...
                    on_save=lambda items: upsert_patient_field_value(page.db_connection, patient_id, insurance_key, json.dumps(items), _list_meta(insurance_key)[0]),
        ...
        sections_column.controls = sections
        try:
            sections_column.update()
        except Exception:
            pass
