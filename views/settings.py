...
def get_settings_view(page: ft.Page, apply_settings_callback):
    ...
    def _show_wipe_prompt():
        if not hasattr(page, "_wipe_dlg") or page._wipe_dlg is None:
            ...
            def close_wipe(_):
                page._wipe_dlg.open = False
                pwd_field.value = ""
                error_txt.value = ""
                try:
                    page._wipe_dlg.update()
                except Exception:
                    pass
                page.update()

            page._wipe_dlg = ft.AlertDialog(
                modal=True,
            ...
            page.overlay.append(page._wipe_dlg)
        
        # Reset fields when reopening
        if hasattr(page._wipe_dlg.content, 'controls'):
            for c in page._wipe_dlg.content.controls:
