...
from utils.ui_helpers import append_dialog, run_async, copy_with_snack, is_sensitive_flag, detect_data_type_from_label, slugify_label, clean_lbl, pt_scale, show_snack

def ensure_sensitive_dialogs_registered(page: ft.Page, *, s, show_snack):
    if getattr(page.mrma, "_sensitive_dlg", None) is not None:
        return
    ...
    page.mrma._ssn_field = ft.TextField(
        label="SSN",
        password=True,
    ...
    def _load_masked():
        ...
        dob_text.value = "DOB: ****-**-**"
        ssn_text.value = "SSN: ***-**-****"
        page.mrma._ssn_field.value = ""
        page.mrma._ssn_field.disabled = True
        status.value = ""
        status.color = ft.Colors.RED
        try:
            page.mrma._sensitive_dlg.update()
        except Exception:
            pass
        page.update()
    ...
    def _reveal(_e=None):
        ...
        page.mrma._sensitive_revealed = True
        dob_text.value = f"DOB: {dob or '(not set)'}"
        ssn_text.value = f"SSN: {ssn or '(not set)'}"
        page.mrma._ssn_field.value = ssn
        page.mrma._ssn_field.disabled = False
        status.value = ""
        status.color = ft.Colors.RED
    ...
    def _save_ssn(_e=None):
        if not getattr(page.mrma, "_sensitive_revealed", False):
        ...
        try:
            ...
            upsert_patient_field_value(
                ...
                (page.mrma._ssn_field.value or "").strip(),
                source="user",
            )
            status.value = "Saved."
            ...
            ssn_text.value = f"SSN: {(page.mrma._ssn_field.value or '').strip() or '(not set)'}"
        except Exception as ex:
    ...
    if page.mrma._sensitive_dlg not in page.overlay:
        append_dialog(page, page.mrma._sensitive_dlg)
...
def open_sensitive_details(page: ft.Page):
    ...
    page.mrma._sensitive_dlg.open = True
    try:
        page.mrma._sensitive_dlg.update()
    except Exception:
        pass
...
def ensure_dialogs_registered(page: ft.Page, *, s, show_snack):
    ...
    """
    if not hasattr(page.mrma, "_dialogs_registered"):
        page.mrma._dialogs_registered = False

    if page.mrma._dialogs_registered:
        return

    # ---------------------------
    # Recovery ceremony dialog
    # ---------------------------
    page.mrma._current_recovery_key = ""  # transient; read by event handler (no closure capture)

    page.mrma._recovery_key_text = ft.Text("", selectable=True, font_family="Consolas", size=pt_scale(page, 14))
    page.mrma._recovery_saved_check = ft.Checkbox(
        label="I saved this recovery key somewhere safe.",
        value=False,
    )
    page.mrma._recovery_status = ft.Text("", color="red", size=pt_scale(page, 12))

    page.mrma._recovery_pending_note = ft.Container(
    ...
    def copy_key(_):
        async def _do():
            ok = await copy_with_snack(
                page,
                page.mrma._current_recovery_key,
            ...
            page.mrma._recovery_status.value = "Copied to clipboard." if ok else "Copy failed on this platform."
            page.mrma._recovery_status.color = ft.Colors.GREEN if ok else ft.Colors.ORANGE

            try:
                page.mrma._recovery_dlg.update()
            except Exception:
                pass
            page.update()
    ...
    def recovery_close(_=None):
        if not page.mrma._recovery_saved_check.value:
            page.mrma._recovery_status.value = "Please confirm you saved it before closing."
            try:
                page.mrma._recovery_dlg.update()
            except Exception:
                pass
        ...
        pending = getattr(page.mrma, "_pending_recovery_rotation", None)
        if pending:
            try:
                ...
                page.mrma._pending_recovery_rotation = None
                show_snack(page, "Recovery key rotation committed.", ft.Colors.GREEN)
            except Exception as ex:
                page.mrma._recovery_status.value = str(ex)
                page.mrma._recovery_status.color = ft.Colors.RED
                try:
                    page.mrma._recovery_dlg.update()
                except Exception:
                    pass
        ...
        # Close dialog first
        page.mrma._recovery_dlg.open = False
        try:
            page.mrma._recovery_dlg.update()
        except Exception:
            pass
        ...
        after = getattr(page.mrma, "_after_recovery_ceremony", None)
        page.mrma._after_recovery_ceremony = None
        if callable(after):
            after()

    page.mrma._recovery_done_btn = ft.Button(
        "I saved it",
        icon=ft.Icons.CHECK,
    ...
    def on_check(_):
        page.mrma._recovery_done_btn.disabled = not bool(page.mrma._recovery_saved_check.value)
        try:
            page.mrma._recovery_dlg.update()
        except Exception:
            pass
    ...
    page.mrma._recovery_dlg = ft.AlertDialog(
        ...
        content=ft.Column(
            [
                ...
                page.mrma._recovery_pending_note,  

                ft.Container(
                    padding=pt_scale(page, 10),
                    border=ft.Border.all(2, ft.Colors.GREY),
                    border_radius=8,
                    content=page.mrma._recovery_key_text,
                ),
                ft.Row([ft.Button("Copy", icon=ft.Icons.CONTENT_COPY, on_click=copy_key)]),
                page.mrma._recovery_saved_check,
                page.mrma._recovery_status,
            ],
            tight=True,
            spacing=pt_scale(page, 10),
        ),
        actions=[page.mrma._recovery_done_btn],
        on_dismiss=recovery_close,
    )
    append_dialog(page, page.mrma._recovery_dlg)

    # ---------------------------
    # Forgot password dialog
    # ---------------------------
    page.mrma._forgot_recovery_field = ft.TextField(
        label="Recovery key",
        password=True,
        can_reveal_password=True,
        width=420,
    )
    page.mrma._forgot_new_pwd_field = ft.TextField(
        label="New database password",
        password=True,
        can_reveal_password=True,
        width=420,
    )
    page.mrma._forgot_new_pwd2_field = ft.TextField(
        label="Confirm new password",
        password=True,
        can_reveal_password=True,
        width=420,
    )
    page.mrma._forgot_status = ft.Text("", color="red")

    def forgot_close(_=None):
        page.mrma._forgot_dlg.open = False
        page.update()

    def do_recover(_):
        rk = (page.mrma._forgot_recovery_field.value or "").strip()
        p1 = page.mrma._forgot_new_pwd_field.value or ""
        p2 = page.mrma._forgot_new_pwd2_field.value or ""

        if not rk:
            page.mrma._forgot_status.value = "Recovery key is required."
            page.update()
            return
        if not p1 or len(p1) < 8:
            page.mrma._forgot_status.value = "New password must be at least 8 characters."
            page.update()
            return
        if p1 != p2:
            page.mrma._forgot_status.value = "Passwords do not match."
            page.update()
            return
        ...
        try:
            ...
            page.mrma._pending_recovery_rotation = {
                "db_path": db_path,
                "dmk_raw": dmk_raw,
            ...
            page.mrma._forgot_dlg.open = False
            try:
                page.mrma._forgot_dlg.update()
            except Exception:
                pass

            # Clear reset fields (good hygiene)
            page.mrma._forgot_recovery_field.value = ""
            page.mrma._forgot_new_pwd_field.value = ""
            page.mrma._forgot_new_pwd2_field.value = ""
            page.mrma._forgot_status.value = ""
            page.update()

            # After ceremony: show message and return to login (if available)
            def _after():
                go_login = getattr(page.mrma, "_go_login", None)
    ...
    page.mrma._forgot_dlg = ft.AlertDialog(
        ...
        content=ft.Column(
            [
                ...
                page.mrma._forgot_recovery_field,
                page.mrma._forgot_new_pwd_field,
                page.mrma._forgot_new_pwd2_field,
                page.mrma._forgot_status,
            ],
            tight=True,
        ),
        actions=[
...
def show_recovery_ceremony(page: ft.Page, recovery_key: str, *, s, show_snack):
    ...
    page.mrma._recovery_pending_note.visible = bool(getattr(page.mrma, "_pending_recovery_rotation", None))

    page.mrma._current_recovery_key = recovery_key
    page.mrma._recovery_key_text.value = recovery_key
    page.mrma._recovery_saved_check.value = False
    page.mrma._recovery_status.value = ""
    page.mrma._recovery_done_btn.disabled = True

    page.mrma._recovery_dlg.open = True
    try:
        page.mrma._recovery_dlg.update()
    except Exception:
        pass
...
def ensure_patient_info_dialogs(page: ft.Page, refresh_callback):
    ...
    if getattr(page.mrma, "_patient_info_dialogs_registered", False):
        page.mrma._patient_info_refresh_callback = refresh_callback
        # Always re-link triggers in case page was rebuilt after an import/routing change
        if not hasattr(page, "open_delete_field_dialog") or not hasattr(page.mrma, "_bulk_edit_dlg"):
            page.mrma._patient_info_dialogs_registered = False  # force full re-registration
        else:
            return
        
    if hasattr(page.mrma, "_bulk_edit_dlg") and hasattr(page.mrma, "_delete_field_dlg"):
        if hasattr(page.mrma, "_impl_open_delete") and hasattr(page.mrma, "_impl_open_bulk"):
            page.open_delete_field_dialog = page.mrma._impl_open_delete
            page.open_add_field_dialog    = page.mrma._impl_open_add
            page.open_bulk_edit_dlg       = page.mrma._impl_open_bulk
        return

    page.mrma._patient_info_refresh_callback = refresh_callback
    if not hasattr(page.mrma, "_revealed_fields"):
        page.mrma._revealed_fields = set() 

    # --- 1. Delete Field Dialog ---
    page.mrma._delete_field_label = ft.Text("")
    
    def _close_delete(_e=None):
        page.mrma._delete_field_dlg.open = False
        try: page.mrma._delete_field_dlg.update()
    ...
    def _confirm_delete(_e=None):
        ...
        table_ref = getattr(page.mrma, "_delete_inline_table", None)
        if row_ref is not None and table_ref is not None:
            try:
                ...
                pass
        page.mrma._delete_inline_row   = None
        page.mrma._delete_inline_table = None
    ...
    append_dialog(page, page.mrma._delete_field_dlg)

    def open_delete_field_dialog(field_key, label):
        page.mrma._delete_field_dlg.target_key = field_key
        page.mrma._delete_field_label.value = f"Field: {label}"
        page.mrma._delete_field_dlg.open = True
        try: page.mrma._delete_field_dlg.update()
        except Exception: pass
        page.update()

    # --- 2. Add Field Dialog ---
    page.mrma._add_field_label_tf = ft.TextField(label="Field label (e.g. Dentist phone)", autofocus=True)
    page.mrma._add_field_category_dd = ft.Dropdown(label="Category", value="Other", width=360)
    page.mrma._add_field_sensitive_cb = ft.Checkbox(label="Hide data by default (Requires click to reveal)", value=False)

    def _close_add(_e=None):
        page.mrma._add_field_dlg.open = False
        try: page.mrma._add_field_dlg.update()
        except Exception: pass
        page.update()
    ...
    def _do_add(_e=None):
        ...
        is_sensitive = 1 if bool(page.mrma._add_field_sensitive_cb.value) else 0
    ...
    append_dialog(page, page.mrma._add_field_dlg)

    def open_add_field_dialog(target_category="Other"):
        page.mrma._add_field_label_tf.value = ""
        page.mrma._add_field_category_dd.value = target_category
        page.mrma._add_field_sensitive_cb.value = False
        try:
        ...
        page.mrma._add_field_category_dd.options = [ft.dropdown.Option(c) for c in merged]
        page.mrma._add_field_dlg.open = True
        try:
            page.mrma._add_field_category_dd.update()
            page.mrma._add_field_dlg.update()
        except Exception: pass
        page.update()
    page.mrma._patient_info_dialogs_registered = True

    # --- 3. Bulk Edit Visibility Dialog ---
    page.mrma._bulk_edit_col = ft.Column(scroll=ft.ScrollMode.AUTO, height=400, spacing=4)
    
    def _close_bulk(_e=None):
        page.mrma._bulk_edit_dlg.open = False
        try: page.mrma._bulk_edit_dlg.update()
        except Exception: pass
        page.update()
    ...
    def _save_bulk(_e=None):
        ...
        for ctrl in _walk(page.mrma._bulk_edit_col):
                ...
                if dtype == "list":
                    ...
                    if key:
                        try:
                            ...
                            if val == 1: page.mrma._revealed_fields.discard(key)
                        except Exception: pass
    ...
    append_dialog(page, page.mrma._bulk_edit_dlg)
    ...
    def open_bulk_edit_dlg():
        ...
        page.mrma._bulk_edit_col.controls.clear()
        page.mrma._bulk_edit_col.controls.append(
            ft.Text("Check a section to enable the visibility toggle features for those fields.", size=12, color="grey")
        )
        page.mrma._bulk_edit_col.controls.append(ft.Container(height=10))
        ...
        def _add_category_section(cat_name: str):
            ...
            page.mrma._bulk_edit_col.controls.append(ft.Container(height=8))
        ...
        page.mrma._bulk_edit_dlg.open = True
        try:
            page.mrma._bulk_edit_dlg.update()
        except Exception:
            pass
        page.update()

    # Link triggers globally to page
    page.mrma._impl_open_delete = open_delete_field_dialog
    page.mrma._impl_open_add    = open_add_field_dialog
    page.mrma._impl_open_bulk   = open_bulk_edit_dlg
