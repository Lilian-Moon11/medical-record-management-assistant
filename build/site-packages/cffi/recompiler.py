...
class Recompiler:
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        prnt('#ifdef PYPY_VERSION')
        prnt('PyMODINIT_FUNC')
        prnt('_cffi_pypyinit_%pt_scale(const void *p[])' % (base_module_name,))
        prnt('{')
        if flags & 1:
        ...
        prnt('     PyMODINIT_FUNC')
        prnt('#  if PY_MAJOR_VERSION >= 3')
        prnt('     PyInit_%pt_scale(void) { return NULL; }' % (base_module_name,))
        prnt('#  else')
        prnt('     init%pt_scale(void) { }' % (base_module_name,))
        prnt('#  endif')
        prnt('#  endif')
        prnt('#elif PY_MAJOR_VERSION >= 3')
        prnt('PyMODINIT_FUNC')
        prnt('PyInit_%pt_scale(void)' % (base_module_name,))
        prnt('{')
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        #
        self._prnt('  %s = %pt_scale(%s%s);' % (tovar, converter, fromvar, extraarg))
    ...
    def _convert_expr_from_c(self, tp, var, context):
        if isinstance(tp, model.BasePrimitiveType):
            if tp.is_integer_type() and tp.name != '_Bool':
                ...
                if cname in ('char16_t', 'char32_t'):
                    self.needs_version(VERSION_CHAR16CHAR32)
                return '_cffi_from_c_%pt_scale(%s)' % (cname, var)
            else:
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        if tp.abi:
            ...
            abi = ''
        name_and_arguments = '%s_cffi_d_%pt_scale(%s)' % (abi, name, repr_arguments)
        ...
        if isinstance(tp.result, model.VoidType):
            result_code = ''
        prnt('  %s%pt_scale(%s);' % (result_code, name, call_arguments))
        prnt('}')
        #
        prnt('#ifndef PYPY_VERSION')        # ------------------------------
        #
        prnt('static PyObject *')
        prnt('_cffi_f_%pt_scale(PyObject *self, PyObject *%s)' % (name, argname))
        prnt('{')
        #
        context = 'argument of %s' % name
        for i, type in enumerate(tp.args):
        ...
        if difference:
            repr_arguments = ', '.join(arguments)
            repr_arguments = repr_arguments or 'void'
            name_and_arguments = '%s_cffi_f_%pt_scale(%s)' % (abi, name,
                                                       repr_arguments)
    ...
    def _struct_decl(self, tp, cname, approxname):
        ...
        prnt('static void %pt_scale(%s *p)' % (checkfuncname, cname))
        prnt('{')
    ...
    def _generate_cpy_const(self, is_int, name, tp=None, category='const',
        ...
        if is_int:
            prnt('static int %pt_scale(unsigned long long *o)' % funcname)
            prnt('{')
            prnt('  int n = (%s) <= 0;' % (name,))
            ...
            prnt('static void %pt_scale(char *o)' % funcname)
            prnt('{')
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        ...
        decl = '*_cffi_var_%pt_scale(void)' % (name,)
        prnt('static ' + tp.get_c_name(decl, quals=self._current_quals))
        prnt('{')
        prnt('  return %pt_scale(%s);' % (ampersand, name))
        prnt('}')
        prnt()
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        repr_arguments = ', '.join(arguments)
        repr_arguments = repr_arguments or 'void'
        name_and_arguments = '%pt_scale(%s)' % (name, repr_arguments)
        if tp.abi == "__stdcall":
